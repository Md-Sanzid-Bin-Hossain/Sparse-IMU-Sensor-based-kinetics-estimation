...
class Kinetics_FM_Net(nn.Module):
    def __init__(self, input_1D,input_2D):
        ...
        self.model_1=Encoder_GRU(input_1D,0.10)
        self.model_2=Encoder_GRU(input_1D,0.10)
        self.model_3=Encoder_GRU(input_1D,0.10)

        self.cnn_1D = Encoder_CNN_1D(input_1D,0.25)
        self.cnn_2D = Encoder_CNN_2D(input_2D,0.25)

                # Outputs
        self.output_GRU = RegularizedLinear(w*512, 7*w)
        self.output_C2 = RegularizedLinear(6*32+w*512, 7*w)
        self.output_C1 = RegularizedLinear(6*32+w*512, 7*w)

        self.l1=nn.Linear(7,128)
        self.l2=nn.Linear(128,7)
    ...
    def forward(self, inputs_1D_N, inputs_2D_N):
        ...
        output_GRU=output_GRU.view(-1,w,7)

        output_C1 = self.output_C1(model_2_output)
        output_C1=output_C1.view(-1,w,7)

        output_C2 = self.output_C2(model_3_output)
        output_C2=output_C2.view(-1,w,7)
...
# Kinetics-FM-DLR-Net-- 1 IMU--Foot
...
def train_sota(train_loader, learn_rate, EPOCHS, model,filename):
    ...
    for epoch in range(EPOCHS):
        ...
        for i, (data_1D,data_2D, data_acc, data_gyr, data_Kinematics,data_EMG,data_JP, target) in enumerate(train_loader):
            ...
            output_1, output_2, output_3, output_rmse,output_4, output_5, output_6, output_pcc,output= model(data_1D[:,:,0:6].to(device).float(),data_2D[:,:,:,0:1].to(device).float())
...
class DL_Kinetics_FM_Net(nn.Module):
    def __init__(self,input_1D,input_2D):
        ...
        self.gru_rmse_1 = nn.GRU(7, 128, batch_first=True)
        self.gru_pcc_1 = nn.GRU(7, 128, batch_first=True)

        self.gru_rmse_2 = nn.GRU(7, 128, batch_first=True)
        self.gru_pcc_2 = nn.GRU(7, 128, batch_first=True)

        self.gru_gain = nn.GRU(256, 128, batch_first=True)
        self.gru_offset = nn.GRU(256, 128, batch_first=True)

        self.dense_gain = nn.Linear(128, 7)
        self.dense_offset = nn.Linear(128, 7)
...
lr = 1e-3
model = DL_Kinetics_FM_Net(6,1)

sota_1 = train_sota(train_loader, lr,40,model,path+'sota_1_imu1.pth')
...
sota_1= DL_Kinetics_FM_Net(6,1)
sota_1.load_state_dict(torch.load(path+'sota_1_imu1.pth'))
sota_1.to(device)
...
with torch.no_grad():
    for i, (data_1D, data_2D, data_acc, data_gyr, data_Kinematics, data_EMG,data_JP, target) in enumerate(test_loader):
        # data_1D=torch.cat((data_1D[:,:,12:18],data_1D[:,:,30:36]),dim=-1)
        # data_2D=torch.cat((data_2D[:,:,:,2:3],data_2D[:,:,:,5:6]),dim=-1)
        output_1, output_2, output_3, output_rmse, output_4, output_5, output_6, output_pcc,output= sota_1(data_1D[:,:,0:6].to(device).float(),data_2D[:,:,:,0:1].to(device).float())
...
rmse, p, Y_1,Y_2,Y_3,Y_4,Y_5,Y_6,Y_7=RMSE_prediction(yhat_4,test_target,s)

ablation_1=np.hstack([rmse,p])

# Kinetics-FM-DLR-Net-- 1 IMU--Foot+ Trunk
...
# Kinetics-FM-DLR-Net-- 1 IMU--Foot+ Trunk +Shank
...
# Kinetics-FM-DLR-Net-- 1 IMU--Foot+ Trunk +Shank+ Thigh
...
lr = 1e-3
model = DL_Kinetics_FM_Net(24,4)

sota_1 = train_sota(train_loader, lr,40,model,path+'sota_1_imu4.pth')
...
sota_1= DL_Kinetics_FM_Net(24,4)
sota_1.load_state_dict(torch.load(path+'sota_1_imu4.pth'))
sota_1.to(device)
...
mm_result_IMU4=np.vstack([ablation_1,ablation_2,ablation_3,ablation_4])


path_1='/home/sanzidpr/Journal_3/Dataset_A_model_results_IMU_emg/Results/'

from numpy import savetxt
savetxt(path_1+subject+'_'+'sota_DL_Kinematics_FM_Net_results.csv', mm_result_IMU4, delimiter=',')
